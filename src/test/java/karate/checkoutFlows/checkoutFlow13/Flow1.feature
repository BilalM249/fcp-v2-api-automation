...
Feature: Checkout with variant product Along with Quantity Base Percentage OFF Promotion
  ...
  Scenario: Quantity_Promotion Applied to Cart with Variant Item
    ...
    * def cart = call read('classpath:karate/core/cart/addToCart.feature@PromotionalCartWithSingleLineItemWithMultiPleQua') {ItemId: '#(ItemId1)', ItemAttributes:'#(ItemId1Attributes)',  quantity:'2' ,InventoryChannelForItem1: '#(inventoryChannelForItem1)'}
