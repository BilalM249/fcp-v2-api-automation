...
Feature: Checkout with  variant and Non Variant  promotional product
  ...
  Scenario:  promotions but no coupons With Variant Item + Non Variant Product
    ...
    * def cart = call read('classpath:karate/core/cart/addToCart.feature@PromotionalCartWithMultiLineItem') {ItemId1: '#(ItemId1)', ItemAttributes1:'#(ItemId1Attributes)', ItemId2: '#(ItemId)', ItemAttributes2:'#(ItemAttributes)',InventoryChannelForItem1: '#(inventoryChannelForItem1)', InventoryChannelForItem2: '#(inventoryChannelForItem2)'}
