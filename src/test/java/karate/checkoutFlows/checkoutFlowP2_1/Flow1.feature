@Sanity
Feature: Checkout with  variant product Where Item get Added to cart Via SKU

  Scenario: No promotions and no coupons With Variant Item Where Item get Added to cart Via SKU
    ...
    * def cart = call read('classpath:karate/core/cart/createCart.feature@AddItemToEmptyCartWithSKU') {sku: '#(sku1)', ItemAttributes:'#(ItemId1Attributes)', InventoryChannelForItem1: '#(inventoryChannelForItem1)'}
    ...
    * def addShipToCart = call read('classpath:karate/core/cart/addShippingToCart.feature@SingleLineItemViaSKU') {cartId: '#(cartId)', shipToId: '#(shipToId)', sku: '#(sku1)'}
