@Sanity
Feature: Checkout with Bundle Product and cancel the complete Order
  Scenario: Checkout with Bundle Product and cancel the complete Order
    ...
    * def cancelOrder = call read('classpath:karate/core/oms/OrderCancel.feature@fullOrderCancel') {orderId : '#(orderId)', shipToId: '#(shipToId)',cartId: '#(cartId)', ItemId: '#(ItemId)', cartTotal: '#(cartAmount)'}
