...
Feature: Checkout with Variant + Non-Variant Product with multi-tier coupon.
  ...
  Scenario: Checkout with Variant + Non-Variant Product with multi-tier coupon.
    ...
    * def applyCoupon = call read('classpath:karate/core/cart/applyCoupon.feature@ApplyingMultiTierCoupon') {cartId: '#(cartId)', couponCode : '#(Variant_Multitier_Quantity_Coupon)'}

    * def cartQuantity = call read('classpath:karate/core/cart/updateQuantity.feature@UpdateQtyForMultiLineItemAndMultiTierQuantityCoupon') {ItemId: '#(ItemId1)', cartId: '#(cartId)', Quantity: '2' }
