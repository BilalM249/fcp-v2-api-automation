@Sanity
Feature: Checkout with Variant Product and promotion is applicable on clearance price.
  Background:
    * def skipTest = karate.env == 'prod01'

  Scenario: Checkout with Variant Product and promotion is applicable on clearance price.
    * print skipTest
    * if (skipTest) karate.abort()
    * def product  = call read('classpath:karate/core/pim/pim.feature@ForVariantItem') {ItemId : '#(Clearance_Promotional_Variant_ItemId)'}
    ...
    * def cart = call read('classpath:karate/core/cart/addToCart.feature@PromotionalCartWithSingleLineItem') {ItemId: '#(ItemId1)', ItemAttributes:'#(ItemId1Attributes)',InventoryChannelForItem1: '#(inventoryChannelForItem1)'}
