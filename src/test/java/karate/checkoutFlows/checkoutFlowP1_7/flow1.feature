...
Feature: Checkout with variant product with expired promotion
  ...
  Scenario:  Checkout with variant product with expired promotion
    ...
    * def cart = call read('classpath:karate/core/cart/addToCart.feature@AddingSingleItemToCartWithExpiredPromotion') {ItemId: '#(ItemId1)', ItemAttributes:'#(ItemId1Attributes)',InventoryChannelForItem1: '#(inventoryChannelForItem1)'}
